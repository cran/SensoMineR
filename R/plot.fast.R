plot.fast=function (x,choix="ind", axes = c(1, 2), xlim = NULL, ylim = NULL, invisible = NULL,
...
#                col.quali.sup <- c(col.quali.sup, rep(aux, nlevels(res.mca$call$X[,
#                  j])))
#                aux = aux + 1
#            }
#        }
...
#            points(coord.ind.sup[, 1], y = coord.ind.sup[, 2],
#                pch = 16, col = col.ind.sup, cex = cex)
#            if (lab.ind.sup)
...
#        plot(0, 0, main = "Supplementary variables on the MCA factor map",
#            xlab = paste("Dim ", axes[1], " (", signif(res.mca$eig[axes[1],
#                2], 4), "%)", sep = ""), ylab = paste("Dim ",
#                axes[2], " (", signif(res.mca$eig[axes[2], 2],
#                  4), "%)", sep = ""), xlim = c(-1.1, 1.1), ylim = c(-1.1,
#                1.1), col = "white", asp = 1, cex = cex)
#        abline(v = 0, lty = 2, cex = cex)
#        abline(h = 0, lty = 2, cex = cex)
...
#            arrows(0, 0, res.mca$quanti.sup$coord[v, 1], res.mca$quanti.sup$coord[v,
#                2], length = 0.1, angle = 15, code = 2, col = col.quanti.sup)
#            if (abs(res.mca$quanti.sup$coord[v, 1]) > abs(res.mca$quanti.sup$coord[v,
#                2])) {
#                if (res.mca$quanti.sup$coord[v, 1] >= 0)
#                  pos <- 4
#                else pos <- 2
#            }
#            else {
#                if (res.mca$quanti.sup$coord[v, 2] >= 0)
#                  pos <- 3
#                else pos <- 1
#            }
#            text(res.mca$quanti.sup$coord[v, 1], y = res.mca$quanti.sup$coord[v,
#                2], labels = rownames(res.mca$quanti.sup$coord)[v],
#                pos = pos, cex = cex, col = col.quanti.sup)
#        }
#    }
group=nrow(res.mca$group$coord)
    if (choix == "group") {
        ...
        #if (!is.null(res.mca$group$coord.sup))
#            coord.illu <- res.mca$group$coord.sup[, axes]
#        if (is.null(col.hab)) {
...
#            col.hab <- (2:(length(group) + 1))
        plot(coord.actif, xlab = paste("Dim ", axes[1], " (", signif(res.mca$eig[axes[1],
                2], 4), "%)", sep = ""), ylab = paste("Dim ", axes[2], " (", signif(res.mca$eig[axes[2],
                2], 4), "%)", sep = ""), xlim = c(0,
            1), ylim = c(0, 1), pch = 17,
