"plotellipseinter" <- function(mat,alpha=0.05,coord=c(1,2),nbbloc=1,moy=TRUE,eig){
...
maxy <- max(coord.ellipse.a.tracer[,2*(1:nbp)],na.rm=TRUE)
  ...
  for (j in 1:nbbloc){
    for (i in 1:nbprod) {
      points(t(X)[nbprod*(j-1)+i,1], t(X)[nbprod*(j-1)+i,2],cex=0.8,col=i,pch=20)
      if (moy==FALSE) lines(c(t(X)[nbprod*(j-1)+i,1],matmoy[i,1]),c( t(X)[nbprod*(j-1)+i,2],matmoy[i,2]),col=i,lty=j)
      lines(coord.ellipse.a.tracer[,(1+2*((i+(j-1)*nbprod)-1)):(2*(i+(j-1)*nbprod))],col=i,lty=j)
    }
  }
