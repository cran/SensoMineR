triangle.pair.test <- function(nb.good,nb.answer){
  ...
  minimum = 0
  for (k in round(nb.answer/3):nb.answer) if ((minimum==0)&(dbinom(k,nb.answer,1/3)<0.05)) minimum=k
  ...
  res = list(p.value=p.value,Estimation=rev(order(aux))[1]-1,ML=round(max(aux),5),minimum=minimum)
  return(res)
}
