...
if ("ind"%in%graph) print(plot(res,cex=1,title=title,color=color,new.plot=FALSE))
if ("var"%in%graph){
  if ("ind"%in%graph & (!nzchar(Sys.getenv("RSTUDIO_USER_IDENTITY"))) ) dev.new()
  print(plot(res,choix="var",cex=1,title=title,color=color,new.plot=FALSE,habillage="group"))
}
result <- list()
result$mfa <- res
if ("ellipse"%in%graph){
  if ((("ind"%in%graph)|("var"%in%graph)) & (!nzchar(Sys.getenv("RSTUDIO_USER_IDENTITY"))))  dev.new()
