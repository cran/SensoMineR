senso.arrow<-function (dfxy, xax = 1, yax = 2, label =
row.names(dfxy), clabel = 1,
    ...
    area = NULL, add.plot = FALSE,col="black",lty=1)
{
    ...
    for (i in 1:(length(coo$x))) arrow1(origin[1], origin[2],
        coo$x[i], coo$y[i], edge = edge,col=col,lty=lty)
    if (clabel > 0)
        senso.scatterutil.eti.circ(coo$x, coo$y, label, clabel, origin,col=col)
    if (csub > 0)
