"senso.corcircle" <-
function (dfxy, xax = 1, yax = 2, label = row.names(df), clabel = 1, grid = TRUE, sub = "", csub = 1, possub = "bottomleft", cgrid = 0, 
    fullcircle = TRUE, box = FALSE, add.plot = FALSE,col="black",lty=1) {
    arrow1 <- function(x0, y0, x1, y1, len = 0.1, ang = 15, lty = 1, edge,col="black") {
        ...
        if (d0 > 2 * h) {
            x0 <- x1 - h * (x1 - x0)/d0
            y0 <- y1 - h * (y1 - y0)/d0
            if (edge)    arrows(x0, y0, x1, y1, ang = ang, len = len, lty = 1,col=col)
        }
    }
    ...
    if (!is.null(label))    showpoint <- FALSE
    x <- df[, xax]
    y <- df[, yax]
    if (add.plot) {
        for (i in 1:length(x)) arrow1(0, 0, x[i], y[i], len = 0.1, ang = 15, edge = TRUE,lty=lty,col=col)
        if (clabel > 0)     senso.scatterutil.eti.circ(x, y, label, clabel,col=col)
        return(invisible())
    }
    ...
    if (grid)     scatterutil.circ(cgrid = cgrid, h = 0.2)
    for (i in 1:length(x)) arrow1(0, 0, x[i], y[i], len = 0.1, ang = 15, edge = TRUE,lty=lty,col=col)
    if (clabel > 0)     senso.scatterutil.eti.circ(x, y, label, clabel, origin,col=col)
